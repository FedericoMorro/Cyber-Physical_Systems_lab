function [x_mse, first_conv, a_mse] = rand_atk(n0, k_fin, thr)

%constant attack on n0 sensors
...
pos_in = randperm(100, 3);  % random starting positions
for j = pos_in
    x_tv(j) = 1;
end

Y_tv = zeros(q, k_fin);     % measurements
atk_sensors = randperm(q, n0);  % sensors under attack
atk_true = zeros(q, 1);         % true value of a represented as usual
for i = atk_sensors
    atk_true(i) = 1;
end
